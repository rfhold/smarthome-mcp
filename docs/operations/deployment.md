...
The runtime contract has six progressive tools. `home_assistant_query` exposes bounded entity, device, state, history, camera, editor-managed scene and automation discovery, exact native config reads, validation, and trace-summary reads. `home_assistant_exec` exposes fixed common controls plus scene and automation upserts. The other four tools expose the bounded catalogs in the [Thread and Matter contract](../home-assistant/spec/thread-matter.md). Entity operations perform a fresh Assist exposure lookup and require explicit `conversation: true`. The eight [authoring and evidence actions](../home-assistant/spec/authoring-evidence.md) deliberately use endpoint-wide `mcp:use` plus the administrator token without exposure checks. System-level Thread and Matter actions use their fixed commands and Matter registry authorization where applicable.

Arbitrary service calls, native Home Assistant MCP bridging, and arbitrary HTTP or WebSocket access are not runtime capabilities. Config deletes, generic config routing, full traces, trace debugging, trigger subscriptions, and script execution remain excluded. The endpoint-wide `mcp:use` scope authorizes all six tools; no per-tool scope separates common controls, authoring, evidence, Thread selection, or Matter interview authority.

Camera, common-control, authoring, evidence, Thread, and Matter support require no additional declared infrastructure resource, deployment manifest, runtime configuration, OAuth configuration, or per-tool grant. The Home Assistant token must be administrator-capable for scene and automation config GETs and POSTs, `trace/list`, Thread commands, and Matter interview. Disposable-instance tests must prove internal API compatibility, discovery, complete reads, replacement, reload scheduling, validation, and trace projection. The recorded preview evidence above predates live authenticated validation of these contracts.
