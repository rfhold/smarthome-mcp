...
The progressive `home_assistant_query` tool is read-only, non-destructive, idempotent, and open-world. It has four actions and always applies the current Assist exposure policy before reading entity data. Root help lists namespaces; namespace help such as `help.device` lists its actions and schemas.
