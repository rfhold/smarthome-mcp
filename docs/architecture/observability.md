...
Action and outcome values pass through fixed allowlists. The action allowlist includes only exact action names, including `camera.snapshot`; it never uses a camera identifier. Cancellation finalizes in-flight telemetry through RAII. REST client outcomes are `success`, `http_error`, `transport_error`, `response_error`, or `cancelled`. `response_error` represents a bounded response-size rejection, not a transport failure. Only 4xx and 5xx status responses set OpenTelemetry HTTP error status; 1xx and 3xx responses retain unset status. REST client spans contain only the request method, response status when available, bounded outcome, and standard application span metadata. Every signal follows the [exposure and data-safety privacy contract](exposure-data-safety.md).
